...
const derivedPlanTargets = computed(() => {
  ...
  allNutrientKeysInPlan.forEach(nutrientKey => {
    ...
    let fdc_id_val = null; // ADDED for fdc_id
    ...
    selectedPeopleObjects.value.forEach(person => {
      ...
      if (personDrvs && personDrvs[nutrientKey]) {
        ...
        if (!fdc_id_val && drvEntry.fdc_id) fdc_id_val = drvEntry.fdc_id; // Capture fdc_id
      }
    });
    ...
    if (nutrientPresentInAtLeastOnePerson) { // Only add to combined if at least one person had this nutrient DRV
      combinedTargets[nutrientKey] = {
        ...
        fdc_id: fdc_id_val // Store fdc_id
      };
    }
  });
  ...
  addedMealComponents.value.forEach((itemInPlan, itemIndex) => {
    ...
    component.ingredientusage_set.forEach(usage => {
      ...
      usage.scaled_nutrient_contributions.forEach(rawContribution => {
        // contribution: { nutrient_name, nutrient_unit, scaled_amount, fdc_nutrient_number (optional), fdc_id (optional) }
        ...
        const { 
          ...
          fdcNumForData, 
          fdcIdForData,
          nutrientKey // THIS IS THE STANDARDIZED KEY (e.g., "Nutrient Name (Unit)")
